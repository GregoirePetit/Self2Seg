          ...
          #  plt.plot(np.array(self.tv), label = "TV")
            if self.iteration %999 == 0:  
              ...
              #  plt.plot(self.lam*np.array(self.tv[498:]))
                plt.legend()
                #plt.show()
                plt.savefig('/home/gpetit/LIP6/codes/Self2Seg/output/energy_fidelity_iteration_'+str(self.iteration)+'.png')
                plt.close()
        self.iteration += 1
...
##################### accelerated segmentation algorithm bg constant############################
    ...
    def segmentation_step2denoisers_acc_bg_constant(self,f, iterations, gt):
       ...
       # diff1 = torch.nn.functional.conv2d(diff2, kernel,padding = 1)
        ...
        #plt.show()
        plt.savefig('/home/gpetit/LIP6/codes/Self2Seg/output/diff1_diff2_'+str(self.iteration_index)+'.png')
        plt.close()
           ...
           # self.x = torch.round(self.x)
            ...
            if i == 9999:  
              ...
              #  plt.plot(self.lam*np.array(self.tv[498:]))
                plt.legend()
                #plt.show()
                plt.savefig('/home/gpetit/LIP6/codes/Self2Seg/output/energy_iteration_'+str(i)+'.png')
                plt.close()
        ...
        while self.notdone: 
            ...
            while (stop == False and epoch<min_epochs):#((timesince < 50 or np.mean(last10psnr)<np.mean(last10psnr_other) or better_than_last == False)) and (stop == False and epoch<min_epochs):
                ...
                if epoch%100 == 0:
                    ...
                    plt.legend()
                    plt.savefig('/home/gpetit/LIP6/codes/Self2Seg/output/psnr_epoch_'+str(epoch)+'.png')
                    #plt.show()
                    plt.close()
